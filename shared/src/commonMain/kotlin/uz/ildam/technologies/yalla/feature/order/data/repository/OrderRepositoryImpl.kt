...
import uz.ildam.technologies.yalla.feature.order.data.mapper.TariffMapper
    ...
    override suspend fun getTariffs(
        ...
        return when (val result = service.getTariffs(
            GetTariffsRequest(
                ...
                coords = coords.map { GetTariffsRequest.Coordination(it.first, it.second) },
                address_id = addressId
            )
        )) {
            is Result.Error -> Result.Error(result.error)
            is Result.Success -> Result.Success(result.data.result.let(TariffMapper.mapper))
        }
    }
}