...
//    val hamburgerButtonState by vm.navigationButtonState.collectAsStateWithLifecycle()
...
//                state.location?.point?.let { mapInstance.animate(it) }
...
//        val processedLocations = remember(state.location, state.destinations) {
//            val start = state.location?.point?.let { MapPoint(it.lat, it.lng) }
...
//                                vm.updateState(state.copy(location = intent.location))
...
//                    cameraButtonState = state.cameraButtonState,
//                    navigationButtonState = hamburgerButtonState,
...
//                                }
//
//                                state.location?.point != null -> {
//                                    state.location?.point?.let { location ->
...
//        } ?: run {
//            state.location?.point?.let { map.moveWithoutZoom(to = it) }
